...
products_path = '/Volumes/e_commerce/default/raw_data_files/products/'
...
products_df = products_df.withColumn('_ingest_timestamp', current_timestamp())\
...
bronze_table_name = f'{bronze_schema}.bronze_products'
...
if spark.catalog.tableExists(bronze_table_name):
    max_event_id = spark.sql(
        f'SELECT COALESCE(MAX(product_id), -1) AS max_id FROM {bronze_table_name}'
    ).collect()[0]['max_id']
else :
    max_event_id = -1
...
products_df = products_df.filter(col('product_id') > max_event_id)
